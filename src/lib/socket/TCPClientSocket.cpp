...
void AsyncIO::TCPClientSocket::OnDataAvailable(std::function<void(TCPClientSocket &)> p_fOnDataAvailableCallback)
{
    m_oAsyncFDIO.OnDataAvailable([this, cb = std::move(p_fOnDataAvailableCallback)]()
                                 { cb(*this); });
}
